...
async function handlePollCreated(event: any, context: handlerContext) {
  ...
  if (!poll) {
    poll = createDefaultPoll(pollId, event.chainId, Number(event.params.pollId));
  }
...
async function handlePollWithdrawn(event: any, context: handlerContext) {
  const creator = event.params.creator;
...
// Handler logic: Voted
async function handlePollVote(event: any, context: handlerContext) {
